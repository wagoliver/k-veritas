...
/**
 * POST /api/projects/[id]/features/[featureId]/generate-tests
 * Gera testes apenas para os cenários revisados dentro desta feature.
 * Escopo granular: menos tokens, mais controle.
 */
export async function POST(
  ...
  // Garante que a feature pertence a este projeto
  ...
  try {
    ...
    await audit({
      ...
      event:
        outcome.status === 'completed'
          ? 'ai_generate_tests_feature_success'
          : 'ai_generate_tests_feature_failure',
      ip: clientIp(req),
    ...
    if (message === 'no_reviewed_scenarios') {
      return Problems.conflict(
        ...
        'Nenhum cenário revisado nesta feature. Revise pelo menos um antes de gerar.',
      )
    }
